...
class TestRunExtract:
    ...
    def test_returns_true_on_success(self, mock_extract):
        """Test that True is returned when extraction succeeds."""
        mock_extract.return_value = {"city_id": {"data": "weather"}}
        
        result = _run_extract("current")
        
        assert result is True
        mock_extract.assert_called_once_with("weather")
    
    @patch('orchestrator.extract_openweathermap')
    def test_returns_false_on_empty_result(self, mock_extract):
    ...
    @patch('orchestrator.extract_openweathermap')
    def test_maps_current_to_weather_endpoint(self, mock_extract):
        """Test that 'current' maps to 'weather' endpoint."""
    ...
    @patch('orchestrator.extract_openweathermap')
    def test_maps_forecast_to_forecast_endpoint(self, mock_extract):
        """Test that 'forecast' maps to 'forecast' endpoint."""
        mock_extract.return_value = {"data": "test"}
